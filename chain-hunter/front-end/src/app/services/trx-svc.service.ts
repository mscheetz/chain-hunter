...
import { TrxAddress10Token } from '../classes/TRX/TrxAddress10Token';
import { TrxAddress20Token } from '../classes/TRX/TrxAddress20Token';
...
export class TrxService{
    ...
    tokensConvert(trx10s: TrxAddress10Token[], trx20s: TrxAddress20Token[]): Asset[] {
        let assets: Asset[] = [];

        trx10s.forEach(token => {
        ...
        trx20s.forEach(token => {
            if(token.hasOwnProperty('symbol') && token.symbol !== null && token.symbol !== ""){
                let asset = new Asset();
                asset.symbol = token.symbol;
                asset.quantity = token.balance.toString();
                assets.push(asset);
     ...
     * @param limit Result quantity
     */
    getTrx10Tokens(page: number, limit: number = 200): Observable<TrxToken10Response>{
        let start = page == 1 ? 0 : ((page - 1) * limit) + 1;
        //let endpoint: string = "/token?sort=-name&limit="+ limit +"&start="+ start +"&totalAll=1&status=ico";
        let endpoint: string = "/token?sort=-name&limit="+ limit +"&start="+ start;// +"&totalAll=1&status=ico";
        let url: string = this.base + endpoint;
